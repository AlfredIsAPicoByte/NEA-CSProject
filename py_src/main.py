...
from src.Raytracing import Raytracer, RayMarchingIntersection, CameraJitterRayGenerator, BasicLambertShading
...
def render_and_save(scene, out_path="render_out.png", spp=1, seed=0):
    tracer = Raytracer(rays_per_pixel=spp)
    sampler = RandomSampler(samples_per_pixel=spp, seed=seed)
    
    # Debug: temporarily make the first object's material emissive so we can confirm rendering output
    if len(scene.objects) > 0 and hasattr(scene.objects[0], "shape") and hasattr(scene.objects[0].shape, "material"):
        scene.objects[0].shape.material.emissive = Color(1.0, 0.3, 0.0)  # visible orange emissive
    
    im = scene.render(tracer, sampler=sampler)
...
if __name__ == "__main__":    # For a quick test, use small resolution; change to 800 for final
    ...
    render_and_save(scene, out_path="render_out.png", spp=1, seed=42)
