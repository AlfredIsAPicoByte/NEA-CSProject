...
class Light:
    ...
    def get_direction(self, light_pos: np.ndarray, hit_point: np.ndarray, direction: np.ndarray) -> np.ndarray:
        """
    ...
    def get_distance(self, light_pos: np.ndarray, hit_point: np.ndarray, bias: float = 1e-6) -> tuple[np.ndarray, float]:
        """
    ...
    def get_radiance(self, light_pos: np.ndarray, hit_point: np.ndarray, direction: np.ndarray) -> Color:
        """
        ...
        if self.type == LightType.SPOT:
            ...
            theta = np.dot(light_to_surface, direction)
