...
def get_gradient_scene(width: int = 64, height: int = 64) -> Scene:
    ...
    # Background Gradient
    sky_colors = [
        Color.from_hex("#2D2515").to_np_array(),
        Color.from_hex("#42424E").to_np_array(),
        Color.from_hex("#5B6791").to_np_array(),
        Color.from_hex("#87BFC6").to_np_array(),
    ]
    sky_positions = [0.0, 0.4, 0.45, 1.0] 

    # Primary Key Light (Sharp, slightly yellow, placed high and to the left for side lighting)
    key_light = LightSource(position=np.array([4.0, 5.0, 0.0]), color=Color.from_hex("#FFEDC7").to_np_array(), intensity=15.0, radius=0.5, name="Key Light")
    
    # Soft Fill Light (Simulates general ambient light or bounce light)
    fill_light = LightSource(position=np.array([-5.0, 2.0, -5.0]), color=Color.from_hex("#C7E5FF").to_np_array(), intensity=3.0, radius=4, name="Fill Light")
    ...
    mat_metal = PBRMaterial.create_specular(Color.from_hex("#47505C").to_np_array(), 0.2, 0.9, 1.0, 1.0)
    ...
    mat_matte = PBRMaterial.create_diffuse(Color.from_hex("#C27A23").to_np_array(), roughness=0.8)
    ...
    mat_glow = PBRMaterial.create_emissive(Color.from_hex("#EE1717").to_np_array(), 2)
...
def get_minimal_scene(width: int = 64, height: int = 64) -> Scene:
    ...
    scene = Scene(name="minimal_scene", camera=cam, background_color=Color.from_hex("#403A43").to_np_array())

    # Sphere at origin
    sphere_shape = Sphere(center=np.array([0.0, 0.0, 0.0]), radius=0.5, name="BallMin")
    mat = PBRMaterial.create_diffuse(Color.from_hex("#227DD7").to_np_array(), 0.2)
    ...
    matg = PBRMaterial.create_diffuse(Color.from_hex("#3F3F3F").to_np_array(), 0.9)
    ground.material = matg
    scene.add_object(VObject(shape=ground, name="GroundMin"))

    # Single light
    light = LightSource(position=np.array([2.0, 3.0, -1.0]), color=Color.from_hex("#FFFFFF").to_np_array(), intensity=15.0, radius=2, name="SunMin")
...
def get_lit_studio_scene(width: int = 100, height: int = 100) -> Scene:
    ...
    scene = Scene(name="lit_studio", camera=cam, background_color=Color.from_hex("#BEC2CF").to_np_array())

    # Objects: two spheres and box as background
    s1 = Sphere(center=np.array([-0.6, 0.4, 0.5]), radius=0.4, name="StudioBallA")
    mat1 = PBRMaterial.create_specular(Color.from_hex("#FFB86B").to_np_array(), 0.2, 0.1, 0.9, 0)
    s1.material = mat1
    scene.add_object(VObject(shape=s1, name="StudioBallA"))

    s2 = Sphere(center=np.array([0.8, 0.45, 0.2]), radius=0.45, name="StudioBallB")
    mat2 = PBRMaterial.create_specular(Color.from_hex("#6B9BFF").to_np_array(), 0.2, 0.4, 0.9, 0)
    s2.material = mat2
    ...
    mat_box = PBRMaterial.create_diffuse(Color.from_hex("#C1CBD0").to_np_array(), roughness=1.0)
    box_shape.material = mat_box
    scene.add_object(VObject(shape=box_shape, name="StudioBox"))

    # Lights
    key = LightSource(position=np.array([2.5, 3.5, -1.0]), color=Color.from_hex("#EEE0BA").to_np_array(), intensity=25.0, radius=100, name="StudioKey")
    key.radius = 0.3
    scene.add_light(key)
    rim = LightSource(position=np.array([-3.0, 2.0, 1.0]), color=Color.from_hex("#DC97C5").to_np_array(), intensity=10.0, radius=0.75, name="StudioRim")
    rim.radius = 0.2
    scene.add_light(rim)
    fill = LightSource(position=np.array([0.0, -2.5, -2.0]), color=Color.from_hex("#C7DBD8").to_np_array(), intensity=15.0, radius=2, name="StudioFill")
    scene.add_light(fill)
...
def get_rgb_room_with_objects_scene(width: int = 126, height: int = 126) -> Scene:
    ...
    # Materials
    mat_white = PBRMaterial.create_diffuse(Color.from_hex("#E0E0E0").to_np_array(), 1.0)
    mat_red   = PBRMaterial.create_diffuse(Color.from_hex("#B03030").to_np_array(), 1.0)
    mat_green = PBRMaterial.create_diffuse(Color.from_hex("#30B030").to_np_array(), 1.0)
    mat_blue = PBRMaterial.create_diffuse(Color.from_hex("#3036B0").to_np_array(), 1.0)
    
    mat_mirror = PBRMaterial.create_specular(Color.from_hex("#FFFFFF").to_np_array(), 0.1, 1.0, 0)
    mat_glass  = PBRMaterial.create_glass(Color.from_hex("#FFFFFF").to_np_array(), Color(1.0, 1.0, 1.0).to_np_array(), 0.0, 0.0, REFRACTIVE_INDICES["glass"], 0)
    ...
    ceiling_light = LightSource(
        ...
        color=Color.from_hex("#FFECDE").to_np_array(), 
        intensity=25.0, 
        radius=1.5, 
        name="CeilingLight"
    )
    ...
    scene = Scene(name="rgb_cornell_box", camera=cam, background_color=Color(0, 0, 0).to_np_array())
...
def get_cyberpunk_scene(width: int = 120, height: int = 120) -> Scene:
    ...
    sky_colors = [
        Color.from_hex("#050008").to_np_array(),
        Color.from_hex("#0B1333").to_np_array(),
    ]
    ...
    mat_wet = PBRMaterial.create_diffuse(Color.from_hex("#151515").to_np_array(), roughness=0.2)
    ...
    mat_chrome = PBRMaterial.create_specular(Color.from_hex("#313238").to_np_array(), roughness=0.2, metallicness=1.0)
    ...
    bldg_right.material = PBRMaterial.create_diffuse(Color.from_hex("#E28335").to_np_array(), roughness=0.9)
    scene.add_object(VObject(shape=bldg_right, name="BldgRight"))

    # Lighting
    light_pink = LightSource(
        position=np.array([-3.0, 2.0, -2.0]), 
        color=Color.from_hex("#FF0099").to_np_array(), 
        intensity=15.0, 
        radius=0.2, 
        name="NeonPink"
    )
    ...
    light_cyan = LightSource(
        ...
        color=Color.from_hex("#00F0FF").to_np_array(), 
        intensity=12.0, 
        radius=0.2, 
        name="NeonCyan"
    )
    ...
    light_blue = LightSource(
        ...
        color=Color.from_hex("#3700FF").to_np_array(), 
        intensity=10.0, 
        radius=0.2, 
        name="NeonBlue"
    )
    ...
    light_rim = LightSource(
        ...
        color=Color.from_hex("#FFFFFF").to_np_array(),
        intensity=5.0,
        radius=0.5,
        name="StreetLight"
    )
...
def get_material_deck_scene(width: int = 160, height: int = 80) -> Scene:
    ...
    scene = Scene(name="material_deck", camera=cam, background_color=Color.from_hex("#000000").to_np_array())

    # Floor
    floor = Cube(center=np.array([0.0, -1.0, 0.0]), side_length=15.0, name="Floor")
    floor.transform.scale = np.array([2.0, 0.1, 1.0])
    floor.material = PBRMaterial.create_diffuse(Color.from_hex("#CCCCCC").to_np_array(), roughness=1.0)
    scene.add_object(VObject(shape=floor, name="Floor"))

    base_col = Color.from_hex("#D4AF37").to_np_array()
    ...
    s5.material = PBRMaterial.create_diffuse(Color.from_hex("#FF0000").to_np_array(), roughness=0.1)
    scene.add_object(VObject(shape=s5, name="S_Plastic"))

    # Lighting
    scene.add_light(LightSource(np.array([0.0, 5.0, -5.0]), Color(1, 1, 1).to_np_array(), 15.0, name="Main"))
    scene.add_light(LightSource(np.array([5.0, 2.0, -2.0]), Color(0.8, 0.8, 1.0).to_np_array(), 10.0, name="Fill"))
...
def get_refraction_lab_scene(width: int = 100, height: int = 100) -> Scene:
    ...
    scene = Scene(name="refraction_lab", camera=cam, background_color=Color(0.05, 0.05, 0.05).to_np_array())
    ...
    wall.material = PBRMaterial.create_emissive(Color(1.0, 1.0, 1.0).to_np_array(), 1.0)
    ...
    for i in range(-6, 7):
        ...
        bar.material = PBRMaterial.create_diffuse(Color(0, 0, 0).to_np_array(), 1.0)
    ...
    s_glass.material = PBRMaterial.create_glass(Color.from_hex("#FFFFFF").to_np_array(), Color(1.0, 1.0, 1.0).to_np_array(), 0.0, 0.0, REFRACTIVE_INDICES["acrylic"], 0)
    ...
    s_diamond.material = PBRMaterial.create_glass(Color.from_hex("#B9D3E3").to_np_array(), Color(0.9, 0.9, 1.0).to_np_array(), 0.0, 0.0, REFRACTIVE_INDICES["diamond"], 0.2)
    ...
    s_water.material = PBRMaterial.create_glass(Color.from_hex("#A6ADD5").to_np_array(), Color.from_hex("#1F1FFF").to_np_array(), 0.0, 0.0, REFRACTIVE_INDICES["water"], 0.1)
    scene.add_object(VObject(shape=s_water, name="WaterSphere"))

    # Lighting
    scene.add_light(LightSource(np.array([2.0, 3.0, -3.0]), Color(1, 1, 1).to_np_array(), 15.0, name="FrontLight"))
