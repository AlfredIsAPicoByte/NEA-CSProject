...
from Geometry import VObject, Sphere
...
def test_world_transform_applied_to_normals():
    ...
    parent = VObject(transform=parent_transform, name="parent")

    # Child sphere at origin (local)
    child_transform = Transform.identity()
    sphere = Sphere()
    child = VObject(transform=child_transform, shape=sphere, name="child")
