...
def get_gradient_scene(width: int = 64, height: int = 64) -> Scene:
    ...
    scene.add_object_by_context(bx_1, "Matte Box", Transform(np.zeros(3), np.array([0.0, np.deg2rad(15), 0.0])))
...
def get_scifi_corridor_scene(width: int = 120, height: int = 120) -> Scene:
    ...
    for z in range(0, -20, -4):
        scene.add_object_by_context(SDF_Material(Cube(), mat_pillar), f"PillarLeft_{z}", Transform(np.array([-2.5, 1.0, z])))
        scene.add_object_by_context(SDF_Material(Cube(), mat_pillar), f"PillarRight_{z}", Transform(np.array([2.5, 1.0, z])))
        scene.add_object_by_context(SDF_Material(Cube(), mat_light_strip), f"Strip_{z}", Transform(np.array([0.0, -0.9, z])))
        
        point_light = Light(color=Color.from_hex("#00AAAA"), intensity=200.0, radius=2.0)
        scene.add_object_by_context(point_light, f"PointLight_{z}", Transform(np.array([0.0, 0.5, z])))
