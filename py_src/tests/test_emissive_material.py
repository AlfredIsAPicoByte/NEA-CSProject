...
def test_emissive_render_direct():
    ...
    sphere = SceneNode(name='E', transform=Transform(position=np.array([0.0, 0.0, 0.0])), shape=Sphere(), material=MaterialFactory.create_emissive(Color.from_hex('#FFAA22'), 1.0))

    scene = Scene(camera=None)
    scene.add_object(sphere)

    tracer = RayTracer(max_recursions=1, intersection_strategy=AnalyticalIntersection(), shading_strategy=LambertShading())
